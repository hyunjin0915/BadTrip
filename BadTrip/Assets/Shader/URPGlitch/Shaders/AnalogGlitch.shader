    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Fragment(Varyings i) : SV_Target
            {
                ...
                half4 src1 = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, frac(float2(u + jitter + shake + drift, jump)));
                half4 src2 = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, frac(float2(u + jitter + shake - drift, jump)));
                half4 src3 = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, frac(float2(u + jitter + shake, jump)));
